...
import com.Hileb.add_potion.api.event.APTooltipEvent;
import com.Hileb.add_potion.common.init.ModConfig;
import com.Hileb.add_potion.common.network.NetworkHandler;
...
public class GuiContainerDemo extends GuiContainer
{
	...
	public void update(TickEvent.WorldTickEvent event) {
		if (event.world.isRemote)coolDown++;
	}

	public GuiContainerDemo(EntityPlayer player,ContainerDemo inventorySlotsIn)
	{
			...
			if(mouseX<=offsetX+23) {
				if (mouseY<=offsetY+13) {
					...
					if (ModConfig.entityElectricShakingConf.ap_showPotion_desc && !foodSlot.getStack().isEmpty()) {
						...
						addPotionTooltip(foodSlot.getStack().copy(), tooltip);
						tooltip.add("");
					}
					if (!potionSlot.getStack().isEmpty()) {
						tooltip.add(I18n.format("com.hileb.ap.desc_2"));
						addPotionTooltip(potionSlot.getStack().copy(), tooltip);
